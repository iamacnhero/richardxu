...
public class ClassUtil {
	...
	private static final Map<Class<?>, Class<?>> primitiveWrapperTypeMap = CollectionUtil
			.getHashMap(8);

	private static final Map<Class<?>, Class<?>> primitiveTypeToWrapperMap = CollectionUtil
			.getHashMap(8);
	...
	protected static class TypeInfo {
		...
		private List<Class<?>> superclasses = CollectionUtil.getArrayList(2);
		private List<Class<?>> interfaces = CollectionUtil.getArrayList(2);
		...
		private TypeInfo(Class<?> type) {
			...
			if (dimension == 0) {
				...
				List<Class<?>> set = CollectionUtil.getArrayList();
	...
	public static Set<Class<?>> getAllInterfacesForClassAsSet(Class<?> clazz,
		...
		if (clazz.isInterface() && isVisible(clazz, classLoader)) {
			Set<Class<?>> set = CollectionUtil.getHashSet(1);
			set.add(clazz);
			return set;
		}
		Set<Class<?>> interfaces = CollectionUtil.getLinkedHashSet();
		while (clazz != null) {
	...
	public static boolean isPresent(String className, ClassLoader classLoader) {
		try {
			ClassLoaderUtil.loadClass(className, classLoader);
			return true;
		} catch (Throwable ex) {
	...
	public static Object newInstance(Class<?> type)
		...
		if (type.isPrimitive()) {
			return newPrimitiveInstance(type);
		}
		...
		if (type == Map.class) {
			return CollectionUtil.getHashMap(1);
		}
		if (type == List.class) {
			return CollectionUtil.getArrayList(1);
		}
		if (type == Set.class) {
			return CollectionUtil.getLinkedHashSet(1);
		}
	...
	private static Object newPrimitiveInstance(Class<?> type) {
