...
import com.richardxu.common.lang.StringPool;
 ...
 * 
 * @version create on 2012-11-5 下午3:39:18
 */
public class PrintfFormat {
	...
	private static final BigInteger bgInt = new BigInteger(
	 ...
	 * 
	 * @param s
	 *            the format string following printf format string The string
	 ...
	 *            <dt>~
	 *            <dd>do not count leading + or - in length
	 *            <dt>0
	 ...
	 *            <dt>,
	 *            <dd>groups decimal values by thousands (for 'diuxXb' formats)
	 *            </dl>
	 * 
	 *            <li>an integer denoting field width (optional)
	 * 
	 *            <li>a period (<b>.</b>) followed by an integer denoting
	 *            precision (optional)
	 * 
	 ...
	 *            <dt>d, i
	 *            <dd>signed long and integer in decimal,
	 *            <dt>u
	 *            <dd>unsigned long or integer in decimal,
	 *            <dt>x
	 *            <dd>unsigned long or integer in hexadecimal,
	 *            <dt>o
	 *            <dd>unsigned long or integer in octal,
	 *            <dt>b
	 *            <dd>unsigned long or integer in binary,
	 *            <dt>s
	 *            <dd>string,
	 *            <dt>c
	 *            <dd>character,
	 *            <dt>l, L
	 *            <dd>boolean in lower or upper case (for booleans and
	 *            int/longs).
	 *            </dl>
	 *            </ul>
	 */
	...
	/**
	 * For internal use with {@link #init(String, int)} and
	 * {@link #reinit(String)}.
	 */
	...
	protected void init(String s, int i) {
		...
		int length = s.length();
		int parseState; // 0 = prefix, 1 = flags, 2 = width, 3 = precision, 4 =
						// format, 5 = end
		...
		while (true) {
			...
			if (i >= length - 1) {
				throw new IllegalArgumentException(
						"Format string can not end with '%'.");
			}
	...
	protected String fixedFormat(double d) {
		...
		if (precision == 0) {
			return (long) (d /* + 0.5 */)
					+ (removeTrailing ? "" : StringPool.DOT); // no rounding
		}
	...
	protected String sign(int s, String r) {
		...
		if (leadingZeroes) {
			w = width;
		} else if ((fmt == 'u' || fmt == 'd' || fmt == 'i' || fmt == 'x'
				|| fmt == 'X' || fmt == 'o')
				&& precision > 0) {
			w = precision;
		}
	...
	/**
	 * Groups numbers by inserting 'separator' after every group of 'size'
	...
	public String form(char value) {
		if (fmt != 'c') {
			throw new IllegalArgumentException("Invalid character format: '"
					+ fmt + "' is not 'c'.");
		}
	...
	public String form(boolean value) {
		...
		throw new IllegalArgumentException("Invalid boolean format: '" + fmt
				+ "' is not one of 'bB'.");
	...
	public String form(double x) {
		...
		if (fmt == 'f') {
			...
			throw new IllegalArgumentException("Invalid floating format: '"
					+ fmt + "' is not one of 'feEgG'.");
		}
		return pad(sign(s, r));
	}
	...
	public String form(long x) {
		...
		case 'L':
			...
			break;
		default:
			throw new IllegalArgumentException("Invalid long format: '" + fmt
					+ "' is not one of 'diuoxXblL'.");
		}
	...
	public String form(int x) {
		...
		case 'L':
			...
			break;
		default:
			throw new IllegalArgumentException("Invalid int format: '" + fmt
					+ "' is not one of 'diuoxXblL'.");
		}
		return pad(sign(s, r));
	}
