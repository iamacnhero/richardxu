...
public class ExceptionHelper {
	...
	static {
		...
		try {
			GET_STACK_TRACE_METHOD = Throwable.class.getMethod(
					GET_STACK_TRACE_METHOD_NAME, new Class[0]);
	...
	public static ChainedThrowable getChainedThrowable(Throwable throwable) {
		if ((throwable != null)
				&& !(ChainedThrowable.class.isInstance(throwable))) {
	 ...
	 * @return 异常的起因
	 */
	public static ChainedThrowable getChainedThrowableCause(
	 ...
	 *            当前的堆栈
	 */
	private static void printStackTraceRecursive(ChainedThrowable throwable,
			PrintWriter writer, String[] currentStack) {
	 ...
	 *            是否是起因异常
	 */
	private static void printThrowableMessage(ChainedThrowable throwable,
			PrintWriter writer, boolean cause) {
	...
	private static String[] analyzeStackTrace(ChainedThrowable throwable) {
		if (GET_STACK_TRACE_METHOD != null) {
			...
			try {
				Object[] stackTrace = (Object[]) GET_STACK_TRACE_METHOD.invoke(
						t, new Object[0]);
	...
	private static class StackTraceAnalyzer {
		...
		private void addLine(String line, boolean includesMessage) {
			StackTraceEntry nextEntry = currentEntry.accept(line,
					includesMessage);
		...
		String[] getLines() {
			return (String[]) selectedEntry.lines
					.toArray(new String[selectedEntry.lines.size()]);
		}
